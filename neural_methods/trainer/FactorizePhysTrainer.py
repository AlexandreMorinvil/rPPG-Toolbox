...
class FactorizePhysTrainer(BaseTrainer):
    ...
    def test(self, data_loader):
        ...
        if self.config.TOOLBOX_MODE == "only_test":
            ...
            self.model.load_state_dict(torch.load(self.config.INFERENCE.MODEL_PATH, map_location=self.device, weights_only=False), strict=False)
            ...
            if self.config.TEST.USE_LAST_EPOCH:
                ...
                self.model.load_state_dict(torch.load(last_epoch_model_path, map_location=self.device, weights_only=False), strict=False)
            else:
                best_model_path = os.path.join(
                    self.model_dir, self.model_file_name + '_Epoch' + str(self.best_epoch) + '.pth')
                print("Testing uses best epoch selected using model selection as non-pretrained model!")
                print(best_model_path)
                self.model.load_state_dict(torch.load(best_model_path, map_location=self.device, weights_only=False), strict=False)
